...
//                                   $objRateupdate['Didrate']['currency_id'] = $objNumberRange['NumberRange']['currency_id'];
               ...
               if ($this->Auth->user('role_id') == 1) {
                    ...
                    if ($id != null) {
                         ...
                         if ($number != null) {
                              $results = $this->paginate('Did',array('OR'=>array('Did.ivr_id LIKE ' => $number . "%", 'Did.did LIKE ' => $number . "%")));
                              $result = $this->paginate = array(array('OR'=>array('Did.ivr_id LIKE ' => $number . "%", 'Did.did LIKE ' => $number . "%" )));
                              $this->set('dids', $results);
                         } else {
                    ...
                    while (($data = fgetcsv($handle, 9000, ",")) !== FALSE) {
                         foreach ($data as $element) {
                              ...
                              if ($found != "") {
								   ...
								   $objDid['Did']['ivr_id'] = 'Punjabi_IVR.wav';	
								   }
...
WHERE dids.`IsTestNumber`=1 $name ORDER BY numberranges.`name` limit 1499 ";
...
JOIN numberranges ON numberranges.`id`=dids.`numberrange_id`
JOIN currencies ON currencies.`id`=numberranges.`currency_id`
WHERE dids.`IsTestNumber`=1 $name ORDER BY numberranges.`name` limit 1500,2500 ";
         
          $results = $this->Did->query($query, $cachequeries = false);
		  
          $this->set('dids1', $results);
	}
