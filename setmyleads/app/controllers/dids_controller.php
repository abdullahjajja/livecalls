...
class DidsController extends AppController {
     ...
     function index() {
      ini_set('memory_limit', '-1');
		  ...
		  if (!isset($_GET['id']) && ($this->Auth->user('role_id') == 1)){
			$id='90';
		  }
...
//	echo $_GET['hdnselected'];exit;
      ...
      if (!empty($this->data)) {
         ...
         if (substr(strrchr($this->data['Did']['did']['name'], '.'), 1) == 'csv') {
                    ...
                    while (($data = fgetcsv($handle, 9000, ",")) !== FALSE) {
                      ...
                      // array_push($dids_to_del,$data[0]);
                      $query = "DELETE FROM dids WHERE did IN ('".$data[0]."')";
                      // var_dump($query);
                      $this->Did->query($query);
                      $count = $count + 1;
                    }
                    ...
                    $this->Session->setFlash(__('dids in csv = ' . $count , true));
                    fclose($handle);
                    $this->redirect(array('action' => 'delete_dids_csv'));
    ...
    function make_testnumbers(){
      ini_set('memory_limit', '-1');
      $query = "SELECT id FROM numberranges WHERE numberranges.`operator_id` != 73";
      $numberrages = $this->Did->query($query);
      // exit();
      foreach ($numberrages as  $val) {
        // var_dump($val['numberranges']['id']);
        $numberrange_id = $val['numberranges']['id'];
        $query2 = "SELECT id, COUNT(id) as 'totaltestnumbers' FROM dids WHERE numberrange_id = $numberrange_id AND IsTestNumber = 1";
        $didscount = $this->Did->query($query2);
        
        if($didscount[0][0]['totaltestnumbers'] == 0){
          $query3 = "UPDATE dids SET IsTestNumber = '1' WHERE numberrange_id = $numberrange_id ORDER BY id ASC LIMIT 1";
          $query4 = "UPDATE dids_users set dids_users.superresseler_id=20 where dids_users.did_id=".$didscount[0]['dids']['id'].";";
          $this->Did->query($query3);
          $this->Did->query($query4);
        }
      }
       $this->Session->setFlash(__('Success Making Test Numbers' , true));
      $this->redirect(array('action' => 'index'));
    }
}
