          ...
          if (!empty($this->data)) {
               ...
               if (substr(strrchr($this->data['Did']['did']['name'], '.'), 1) == 'csv') {
                    ...
                    while (($data = fgetcsv($handle, 9000, ",")) !== FALSE) {
                        // if($count!=0)
                        // {
							if($rangeName == "" or $rangeName != $data[0]) {
								$count = 0;
								$rangeName = $data[0];
							}
...
//		$numberranges = $this->Did->Numberrange->find('list');
     ...
     function ratecard() {
       ...
       if ($name == "" && isset($_GET['alpha'])) {
        	  $str=$_GET['alpha'];
               $name ="and (numberranges.`name` like '$str%')";
          }	else {
		       $name ="and (numberranges.`name` like 'A%')";
		 }		  
//		echo "<br>>>" . $name . "<<<br>";
...
//         $this->paginate = array('order' => array('Numberrange.name' => 'asc'));
/*		$this->paginate = array('limit' => 100);
         $this->paginate = array($this->Did->query($query, $cachequeries = false));
         $this->set('dids', $this->paginate()); */
