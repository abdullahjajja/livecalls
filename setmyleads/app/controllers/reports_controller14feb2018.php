...
class ReportsController extends AppController {
    ...
    function access_cdr(){
      ...
      if(isset($this->params['form']['caller_id_num']) && !empty($this->params['form']['caller_id_num'])){
        ...
        $query          = "SELECT cdrs.`caller_id_name`,COALESCE(NULLIF(cdrs.`numberrange_name`,''),'N/A') AS 'numberrange_name', COALESCE(numberranges.`sellingrate`,'N/A') AS 'sellingrate', numberranges.`id`,COALESCE(dids.`did`, 'N/A') AS 'Testnumber' 
                        FROM cdrs
                        ...
                        WHERE cdrs.`caller_id_name` LIKE '{$caller_id_num}%' AND start_stamp  >= DATE_SUB(NOW(),INTERVAL 1 DAY)
                        AND cdrs.`billsec`= 0
