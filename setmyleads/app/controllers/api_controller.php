...
//          echo $file;
     ...
     function export_numberranges() {
          ...
          $query = "SELECT  operators.`name` as 'operater name', numberranges.`name` , route , ivrpath , currencies.`currency_name` , clilimit , buyingrate ,dailyrate, sellingrate AS 'Weekly Rate', `monthlyrate` AS 'Monthly Rate',minduraction AS 'min duration' ,maxduration , calllimit , maxdailyminutes,DATE(numberranges.`created`) AS 'Creation Date' FROM numberranges
JOIN currencies ON currencies.`id`=numberranges.`currency_id`
LEFT JOIN operators ON numberranges.`operator_id` = operators.`id`
ORDER BY operators.`name` , numberranges.`name` ";
          $data = mysql_query($query) or mysql_error();
...
//          echo $file;
     ...
     function export_dids() {
          ...
          $query = "SELECT numberranges.`name` AS 'Range Name', did AS 'Did',dids.`created` AS 'Created Date',numberranges.`buyingrate` AS 'Buying Rate', numberranges.`sellingrate` AS 'Selling Rate', CASE numberranges.`currency_id`  WHEN 1 THEN 'USD' WHEN 2 THEN 'EURO' WHEN 4 THEN 'GPB' ELSE 'Any' END AS 'Currency', IsTestNumber AS 'Test Number',mincallduration AS 'Minimum Call Duration',perclilimit,dids.maxdailyminutes AS 'Max Daily Minutes',
...
//          echo $file;
     ...
     function downladaccessnetworkxlsx() {
          $this->autoRender = false;
          $path = WWW_ROOT;
          $latest_filename = 'cdrs_access.xlsx';
          $file = "../../app/webroot/" . $latest_filename;
          if (file_exists($latest_filename)) {
     ...
     function download_noanswer_cdrs()
     {
          $this->autoRender = false;

          $path = WWW_ROOT;

          $query = "SELECT unmatch_did_cdrs.`caller_id_name`, unmatch_did_cdrs.caller_id_number, unmatch_did_cdrs.destination_number, unmatch_did_cdrs.start_stamp, (unmatch_did_cdrs.billsec-unmatch_did_cdrs.billsec * (unmatch_did_cdrs.Addon/100)) AS billsec , 
unmatch_did_cdrs.answer_stamp, COALESCE(NULLIF(unmatch_did_cdrs.`numberrange_name`,''),'N/A') AS 'numberrange_name', unmatch_did_cdrs.`operator_ip`
 FROM unmatch_did_cdrs
      WHERE answer_stamp = '0000-00-00 00:00:00' 
                AND start_stamp  >= DATE_SUB(NOW(),INTERVAL 1 DAY)
                GROUP BY unmatch_did_cdrs.`destination_number`, unmatch_did_cdrs.`operator_ip`
          ...
          $nn = date('ymd');
          // $file = $_SERVER['SERVER_ADDR'] . "/livecalls/app/webroot/weekly.csv";
          $filename = "../../app/webroot/noanswercdr.csv";
          ...
          //echo $file;exit;
          // $file = "../app/webroot/numberranges.csv";
          $latest_filename = 'noanswercdr.csv';
//          echo $file;
          $file = "../../app/webroot/" . $latest_filename;
          if (file_exists($latest_filename)) {
...
$response = $gcpm->send($messageNotify, array('title' => 'Request for Number assignment'));
          ...
          if (count($call) > 0) {
               ...
               if ($count == 0) {
                  ...
                  $messagefornotification="$count numbers from $name have been added on $term payment term From Rate Card on this Rate ".$objRateupdate['Didrate']['superresrate'];
...
//</div>';
     ...
     function getCalls() {
          ...
          // if super resseller viewing calls
          if ($this->Auth->user('role_id') == 2) {
               $query = "SELECT channels.ip_addr,channels.created,channels.dest,channels.cid_num,TIMEDIFF(time(now()),time(channels.created))as Duration,dids_users.superresseler_id,
     ...
     or users.created_by = " . $this->Auth->user('id') . " and dids.IsTestNumber=0 and channels.direction='inbound' ORDER BY channels.created DESC ;";
          ...
          // admin viewing calls
               ...
               $query = "SELECT SUBSTRING_INDEX(channels.presence_id,'@',-1) AS ip_addr,channels.created,channels.dest,channels.cid_num,TIMEDIFF(time(now()),time(channels.created))as Duration,dids_users.superresseler_id,dids_users.resseller_id,numberranges.name, dids_users.subresseller_id FROM livecalls.dids
     INNER JOIN livecalls.dids_users ON (dids.id = dids_users.did_id)
     inner join numberranges on numberranges.id=dids.numberrange_id
     INNER JOIN livecalls.channels ON (channels.dest = dids.did) where dids.IsTestNumber=0  and channels.direction='inbound' ORDER BY channels.created DESC ;";
          ...
          $strCalls = "<div id='maincontent'><div class=\"mainheading\"><div></div>" .
				...
				$strCalls=$strCalls      . 	"<div style=\"float:left;\"><a href=\"../report/\" >Report</a> <a href=\"../report/operator\" >Operator Report</a></div>";
                    $strOper = "<td width=\"15%\" align=\"left\" style=\"padding-left:10px;\">Operator</td>";  
                    }
             $strCalls=$strCalls      . " </div></div> </div>" .
                  ...
                  "<td width=\"15%\" align=\"left\" style=\"padding-left:10px;\">Called Number</td>" .
                  "<td width=\"15%\" align=\"center\">Duration</td>" .
                  "<td width=\"15%\" align=\"left\" style=\"padding-left:10px;\">Calling Number</td>" .
                  $strOper .
                  "</tr>" .
                  "</table></td>" .
                  "</tr>";

          App::import('Helper', 'Getoperator');
          $getoperator = new getoperatorHelper();

          App::import('Helper', 'Getuser');
          ...
          if (count($calls) > 0) {
               ...
               foreach ($calls as $call) {
                    ...
                    if ($this->Auth->user('role_id') == 1) {
                         ...
                         $oper_name = $getoperator->get_oper_name_by_ip($call[0]['ip_addr']);
                         $strOperValue = "<td width=\"15%\"align=\"left\" class=\"gridcellborder\" style=\"padding-left:10px;\">" . $oper_name . "</td>";
                    ...
                    $strCalls = $strCalls . "<tr" . $class . "><td><table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\">" .
                            ...
                            "<td width=\"15%\" align=\"left\" class=\"gridcellborder\" style=\"padding-left:10px;\">" . $call['channels']['dest'] . "</td>" .
                            "<td width=\"15%\" align=\"center\" class=\"gridcellborder\">" . $call['0']['Duration'] . "</td>" .
                            "<td width=\"15%\"align=\"left\" class=\"gridcellborder\" style=\"padding-left:10px;\">" . $call['channels']['cid_num'] . "</td>" .
                            $strOperValue.
                            "</tr>" .
                            "</table></td>
                                             </tr>";
    ...
    function getTestCalls() {
     ...
     function getTNActiveCalls() {
          ...
          if ($this->Auth->user('role_id')) {
               ...
               // if super resseller viewing calls
               if ($this->Auth->user('role_id') == 2) {
                    $query = "SELECT numberranges.`name`, channels.created,channels.dest,channels.cid_num,TIMEDIFF(time(now()),time(channels.created))as Duration FROM livecalls.dids
                     INNER JOIN livecalls.channels ON (channels.dest = dids.did) INNER JOIN numberranges ON (numberranges.id=dids.`numberrange_id`) where dids.IsTestNumber=1 and channels.direction='inbound';";
               }
               // if resseller viewing calls
               else if ($this->Auth->user('role_id') == 3) {
                    $query = "SELECT numberranges.`name`, channels.created,channels.dest,channels.cid_num,TIMEDIFF(time(now()),time(channels.created))as Duration FROM livecalls.dids
               ...
               $strCalls = "<div class=\"mainheading\"><div></div>" .
                       ...
                       "<td width=\"10%\" align=\"center\">Sr.No</td>" .
                        "<td width=\"25%\" align=\"center\">Number Range</td>" .
                       "<td width=\"25%\" align=\"center\">Called Number</td>" .
                       "<td width=\"15%\" align=\"center\">Duration</td>" .
                       "<td width=\"25%\" align=\"center\">Calling Number</td>" .
                       "</tr>" .
                       "</table></td>" .
                       "</tr>";
               ...
               if (count($calls) > 0) {
                    foreach ($calls as $call) {
                         ...
                         $strCalls = $strCalls . "<tr" . $class . "><td><table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\">" .
                                 ...
                                 "<td width=\"25%\" align=\"center\" class=\"gridcellborder\">" . $call['numberranges']['name'] . "</td>" .
                                 "<td width=\"25%\" align=\"center\" class=\"gridcellborder\">" . $call['channels']['dest'] . "</td>" .
                                 "<td width=\"15%\" align=\"center\" class=\"gridcellborder\">" . $call['0']['Duration'] . "</td>" .
                                 "<td width=\"25%\" align=\"center\" class=\"gridcellborder\">" . preg_replace('/\d{5}$/', 'xxxxx', $call['channels']['cid_num']) . "</td>" .
                                 "</tr>" .
                                 "</table></td>
                                                       </tr>";
                         $j = $j + 1;
...
$conditonOnTerm="cdrs.isdaily=$payment_term and";	
          ...
          $query = "select cdrs.`currency_id`, cdrs.`isdaily`,cdrs.operator_name, cdrs.`superresrate`, cdrs.`ressellerrate` , cdrs.`subresrate` ,users.`login`,cdrs.caller_id_name, cdrs.caller_id_number, cdrs.destination_number, cdrs.start_stamp, (cdrs.billsec-cdrs.billsec * (cdrs.Addon/100)) as billsec , cdrs.answer_stamp, cdrs.numberrange_name, cdrs.adminbuyrate from cdrs ";
...
//if super reseller viewing calls
          ...
          $strCalls = "<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"10\"><tr><td align=\"left\" valign=\"top\">" .
                  ...
                  "<td width=\"10%\" align=\"left\" style=\"padding-left:5px;\">Customer </td>";
                  if($this->Auth->user('role_id') == 1)
               { 
                $strCalls= $strCalls.   "<td width=\"8%\" align=\"left\" style=\"padding-left:5px;\">Operator Name </td>";
                $strCalls= $strCalls.   "<td width=\"7%\" align=\"left\" style=\"padding-left:5px;\">Buying Rate </td>";
               } 
               $strCalls= $strCalls."<td width=\"3%\" align=\"left\" style=\"padding-left:5px;\">Cur </td>";
	...
	break;
                    ...
                    $strCalls = $strCalls . "<tr" . $class . "><td class=\"grid1light\"><table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\">" .
                            ...
                            "<td width=\"12%\" align=\"left\" style=\"padding-left:5px;\" class=\"gridcellborder\" >". $call['cdrs']['numberrange_name'] ."</td>" . "<td width=\"10%\" align=\"left\" style=\"padding-left:5px;\" class=\"gridcellborder\">" . $call['users']['login'] . " </td>" ;
                            if($this->Auth->user('role_id') == 1)
                    {
						 $strCalls = $strCalls . "<td width=\"8%\" align=\"left\" class=\"gridcellborder\" style=\"padding-left:5px;\">" . $call['cdrs']['operator_name']. "</td>";
                                $strCalls = $strCalls . "<td width=\"7%\" align=\"left\" class=\"gridcellborder\" style=\"padding-left:5px;\">" . $call['cdrs']['adminbuyrate']. "</td>";
					}
                         $strCalls = $strCalls . "<td width=\"3%\" align=\"left\" class=\"gridcellborder\" style=\"padding-left:5px;\">". $getcurrency->getcurrencyNameById($call['cdrs']['currency_id'])  ."</td>";
...
$conditonOnTerm="cdrs.isdaily=$payment_term and";	
          ...
          if ($this->Auth->user('role_id') == 2) {
               ...
               if ($subcustomer == -1 && $destination == -1) {
                    ...
                    $query = $query . " WHERE $conditonOnTerm (cdrs.superresseler_id = " . $this->Auth->user('id') . ") and cdrs.start_stamp BETWEEN " . "'" . $start . "'" . " and " . "'" . $end . ":59'" . " GROUP BY cdrs.destination_number, cdrs.superresseler_id,cdrs.isdaily,cdrs.`superresrate`,cdrs.`Addon`, cdrs.resseller_id";
               } else if ($subcustomer == -1 && $destination != -1) {

                    $query = $query . " WHERE $conditonOnTerm cdrs.numberrange_name=" . "'" . $destinationtxt . "'" . " and (cdrs.superresseler_id = " . $this->Auth->user('id') . " ) and cdrs.start_stamp BETWEEN " . "'" . $start . "'" . " and " . "'" . $end . ":59'" . " GROUP BY cdrs.destination_number, cdrs.superresseler_id,cdrs.isdaily,cdrs.`superresrate`,cdrs.`Addon`, cdrs.resseller_id";
               } else if ($subcustomer != -1 && $destination == -1) {

                    $query = $query . " WHERE $conditonOnTerm cdrs.resseller_id=" . $subcustomer . " and (cdrs.superresseler_id = " . $this->Auth->user('id') . " ) and cdrs.start_stamp BETWEEN " . "'" . $start . "'" . " and " . "'" . $end . ":59'" . " GROUP BY cdrs.destination_number, cdrs.superresseler_id,cdrs.isdaily,cdrs.`superresrate`,cdrs.`Addon`, cdrs.resseller_id";
               } else if ($subcustomer != -1 && $destination != -1) {
                    $query = $query . " WHERE $conditonOnTerm cdrs.resseller_id=" . $subcustomer . " and cdrs.numberrange_name=" . "'" . $destinationtxt . "'" . " and (cdrs.superresseler_id = " . $this->Auth->user('id') . ") and cdrs.start_stamp BETWEEN " . "'" . $start . "'" . " and " . "'" . $end . ":59'" . " GROUP BY cdrs.destination_number, cdrs.superresseler_id,cdrs.isdaily,cdrs.`superresrate`,cdrs.`Addon`, cdrs.resseller_id";
               }
          }
          // if resseller viewing calls
          else if ($this->Auth->user('role_id') == 3) {
               ...
               if ($subcustomer == -1 && $destination == -1) {
                    ...
                    $query = $query . " WHERE (cdrs.resseller_id = " . $this->Auth->user('id') . " ) and cdrs.start_stamp BETWEEN " . "'" . $start . "'" . " and " . "'" . $end . ":59'" . "GROUP BY cdrs.destination_number,cdrs.isdaily,cdrs.`Addon`";
               } else if ($subcustomer == -1 && $destination != -1) {

                    $query = $query . " WHERE cdrs.numberrange_name=" . "'" . $destinationtxt . "'" . " and (cdrs.resseller_id= " . $this->Auth->user('id') . " ) and cdrs.start_stamp BETWEEN " . "'" . $start . "'" . " and " . "'" . $end . ":59'" . " GROUP BY cdrs.destination_number,cdrs.isdaily,cdrs.`Addon`, cdrs.subresseller_id";
               } else if ($subcustomer != -1 && $destination == -1) {

                    $query = $query . " WHERE cdrs.subresseller_id=" . $subcustomer . " and (cdrs.resseller_id= " . $this->Auth->user('id') . ") and cdrs.start_stamp BETWEEN " . "'" . $start . "'" . " and " . "'" . $end . ":59'" . " GROUP BY cdrs.destination_number,cdrs.isdaily,cdrs.`Addon`, cdrs.subresseller_id";
               } else if ($subcustomer != -1 && $destination != -1) {
                    $query = $query . " WHERE cdrs.subresseller_id=" . $subcustomer . " and cdrs.numberrange_name=" . "'" . $destinationtxt . "'" . " and (cdrs.resseller_id= " . $this->Auth->user('id') . " ) and cdrs.start_stamp BETWEEN " . "'" . $start . "'" . " and " . "'" . $end . ":59'" . " GROUP BY cdrs.destination_number,cdrs.isdaily,cdrs.`Addon`, cdrs.subresseller_id";
               }
          ...
          // if sub resseller viewing calls
               ...
               if ($subcustomer == -1 && $destination == -1) {
                    ...
                    $query = $query . " WHERE (cdrs.subresseller_id= " . $this->Auth->user('id') . " ) and cdrs.start_stamp BETWEEN " . "'" . $start . "'" . " and " . "'" . $end . ":59'" . "GROUP BY cdrs.destination_number,cdrs.isdaily,cdrs.`Addon`";
               } else if ($subcustomer == -1 && $destination != -1) {

                    $query = $query . " WHERE cdrs.numberrange_name=" . "'" . $destinationtxt . "'" . " and (cdrs.subresseller_id= " . $this->Auth->user('id') . " ) and cdrs.start_stamp BETWEEN " . "'" . $start . "'" . " and " . "'" . $end . ":59'" . " GROUP BY cdrs.destination_number,cdrs.isdaily,cdrs.`Addon`";
               } else if ($subcustomer != -1 && $destination == -1) {
          ...
          // admin viewing CDR
               ...
               $query = "SELECT cdrs.isdaily,cdrs.operator_name, users.first_name,users.last_name,cdrs.destination_number,Round(Round(SUM(cdrs.billsec)/60,2)-Round(SUM(cdrs.billsec)/60,2)*(cdrs.Addon/100),2) AS duration,
                    COUNT(cdrs.start_stamp) AS totatcalls, cdrs.numberrange_name as destination, currencies.currency_name, cdrs.superresrate, cdrs.adminbuyrate FROM cdrs
                    INNER JOIN users ON cdrs.superresseler_id=users.id AND users.`id` != 23 AND users.`id` !=4242
                    LEFT Join currencies on currencies.id = cdrs.currency_id ";
               /*
               ...
               if ($subcustomer == -1 && $destination == -1) {
                    ...
                    $query = $query . " WHERE $conditonOnTerm cdrs.start_stamp BETWEEN " . "'" . $start . "'" . " and " . "'" . $end . ":59'" . " and cdrs.billsec > 0  GROUP BY cdrs.destination_number,cdrs.superresseler_id,cdrs.isdaily,cdrs.`superresrate`,cdrs.`Addon`,cdrs.`adminbuyrate`, cdrs.`currency_id`";
               } else if ($subcustomer == -1 && $destination != -1) {

                    $query = $query . " WHERE $conditonOnTerm  cdrs.numberrange_name=" . "'" . $destinationtxt . "'" . " and (users.id = " . $this->Auth->user('id') . " or users.created_by= " . $this->Auth->user('id') . " ) and cdrs.start_stamp BETWEEN " . "'" . $start . "'" . " and " . "'" . $end . ":59'" . " cdrs.billsec > 0 GROUP BY cdrs.destination_number, cdrs.superresseler_id,cdrs.isdaily,cdrs.`superresrate`,cdrs.`Addon`,cdrs.`adminbuyrate`,cdrs.`currency_id";
               } else if ($subcustomer != -1 && $destination == -1) {

                    $query = $query . " WHERE $conditonOnTerm  cdrs.superresseler_id=" . $subcustomer . " and cdrs.start_stamp BETWEEN " . "'" . $start . "'" . " and " . "'" . $end . ":59'" . " cdrs.billsec > 0  GROUP BY cdrs.destination_number, cdrs.superresseler_id,cdrs.isdaily,cdrs.`superresrate`,cdrs.`Addon`,cdrs.`adminbuyrate`,cdrs.`currency_id";
               } else if ($subcustomer != -1 && $destination != -1) {
                    $query = $query . " WHERE $conditonOnTerm  cdrs.superresseler_id=" . $subcustomer . " and cdrs.numberrange_name=" . "'" . $destinationtxt . "'" . " and cdrs.start_stamp BETWEEN " . "'" . $start . "'" . " and " . "'" . $end . ":59'" . "cdrs.billsec > 0  GROUP BY cdrs.destination_number, cdrs.superresseler_id,cdrs.isdaily,cdrs.`superresrate`,cdrs.`Addon`,cdrs.`adminbuyrate`,cdrs.`currency_id";
               }
          }
...
//		  echo $query;
          ...
          $strCalls = "<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"10\">" .
               ...
               { 
                ...
                "<td width=\"11%\" height=\"29px\" align=\"left\" style=\"padding-left:10px;\">Number </td>" .
                  "<td width=\"7%\" align=\"right\" style=\"padding-right:10px;\">Calls</td>" .
                  "<td width=\"7%\" align=\"right\" style=\"padding-right:10px;\">Minutes</td>" .
                  "<td width=\"10%\" align=\"left\" style=\"padding-left:10px;\">Payment Terms</td>" .
                  "<td width=\"7%\" align=\"center\">Currency</td>" .
                  "<td width=\"7%\" align=\"right\" style=\"padding-right:10px;\">Sell Rate</td>" ;
                  if($this->Auth->user('role_id') == 1)
               { 
                $strCalls= $strCalls.   "<td width=\"7%\" height=\"29px\" align=\"left\" style=\"padding-left:10px;\">Buy Rate</td>";
               } 
                  $strCalls= $strCalls."</tr>" .
                  "</table></td>" .
                  "</tr>" .
                  "<tr>" .
                  "</tr>" .
	...
	break;
                    ...
                    $strCalls = $strCalls . "<td width=\"14%\" height=\"29px\" align=\"left\" class=\"gridcellborder\" style=\"padding-left:10px;\">". $call['cdrs']['destination'] ."</td>" .
                            "<td width=\"11%\" height=\"29px\" align=\"left\" style=\"padding-left:10px;\" class=\"gridcellborder\">" . $call['cdrs']['destination_number'] . "</td>" .
                            //"<td width=\"12%\" align=\"center\" class=\"gridcellborder\"></td>".
                            ...
                            "<td width=\"7%\" align=\"right\" style=\"padding-right:10px;\" class=\"gridcellborder\">" . $call['cdrs']['superresrate'] . " </td>" .
                             "<td width=\"7%\" align=\"right\" style=\"padding-right:10px;\" class=\"gridcellborder\">" . $call['cdrs']['adminbuyrate'] . " </td>" .
                            //"<td width=\"7%\" align=\"center\">-</td>".
                            "</tr>" .
                            "</table></td>" .
                            "</tr>";
...
$conditonOnTerm="cdrs.isdaily=$payment_term and";	
          ...
          // admin viewing CDR
               ...
               if ($subcustomer == -1 && $destination == -1) {
                    ...
                    $query = $query . " WHERE $conditonOnTerm cdrs.start_stamp BETWEEN " . "'" . $start . "'" . " and " . "'" . $end . ":59'" . " GROUP BY cdrs.destination_number,cdrs.superresseler_id,cdrs.isdaily,cdrs.`superresrate`,cdrs.`adminbuyrate`, cdrs.`currency_id`";
               } else if ($subcustomer == -1 && $destination != -1) {

                    $query = $query . " WHERE $conditonOnTerm  cdrs.numberrange_name=" . "'" . $destinationtxt . "'" . " and (users.id = " . $this->Auth->user('id') . " or users.created_by= " . $this->Auth->user('id') . " ) and cdrs.start_stamp BETWEEN " . "'" . $start . "'" . " and " . "'" . $end . ":59'" . " GROUP BY cdrs.destination_number,cdrs.superresseler_id,cdrs.isdaily,cdrs.`superresrate`,cdrs.`adminbuyrate`, cdrs.`currency_id`";
               } else if ($subcustomer != -1 && $destination == -1) {

                    $query = $query . " WHERE $conditonOnTerm  cdrs.superresseler_id=" . $subcustomer . " and cdrs.start_stamp BETWEEN " . "'" . $start . "'" . " and " . "'" . $end . ":59'" . " GROUP BY cdrs.destination_number,cdrs.superresseler_id,cdrs.isdaily,cdrs.`superresrate`,cdrs.`adminbuyrate`, cdrs.`currency_id`";
               } else if ($subcustomer != -1 && $destination != -1) {
                    $query = $query . " WHERE $conditonOnTerm  cdrs.superresseler_id=" . $subcustomer . " and cdrs.numberrange_name=" . "'" . $destinationtxt . "'" . " and cdrs.start_stamp BETWEEN " . "'" . $start . "'" . " and " . "'" . $end . ":59'" . " GROUP BY cdrs.destination_number,cdrs.superresseler_id,cdrs.isdaily,cdrs.`superresrate`,cdrs.`adminbuyrate`, cdrs.`currency_id`";
               }
          }
               ...
               { 
                $strCalls= $strCalls.   "<td width=\"14%\" height=\"29px\" align=\"left\" style=\"padding-left:10px;\">Operator Name </td>";
                 ...
                 $strCalls= $strCalls.  "<td width=\"14%\" height=\"29px\" align=\"left\" style=\"padding-left:10px;\">Number Range </td>" .
                  ...
                  "<td width=\"7%\" align=\"right\" style=\"padding-right:10px;\">Calls</td>" .
                  "<td width=\7%\" align=\"right\" style=\"padding-right:10px;\">Minutes</td>" .
                  "<td width=\"7%\" align=\"right\" style=\"padding-right:10px;\">Curr.</td>" .
                  "<td width=\"5%\" align=\"right\" style=\"padding-right:10px;\">Sell Rate</td>";
                  if($this->Auth->user('role_id') == 1)
               { 
                $strCalls= $strCalls.   "<td width=\"7%\" height=\"29px\" align=\"left\" style=\"padding-left:10px;\">Buy Rate</td>";
               } 
	...
	break;
                    ...
                    $strCalls = $strCalls . "<tr" . $class . "><td class=\"grid1light\"><table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\">" .
                            "<tr>" .
                            "<td width=\"14%\" height=\"29px\" align=\"left\" class=\"gridcellborder\" style=\"padding-left:10px;\">" . $call['users']['first_name'] . " " . $call['users']['last_name'] . "</td>";
                            if($this->Auth->user('role_id') == 1)
                    {
                               $strCalls = $strCalls . "<td width=\"14%\" height=\"29px\" align=\"left\" class=\"gridcellborder\" style=\"padding-left:10px;\">" . $call['cdrs']['operator_name']. "</td>";
                         }
                      $strCalls = $strCalls .      "<td width=\"14%\" height=\"29px\" align=\"left\" class=\"gridcellborder\" style=\"padding-left:10px;\">" . $call['cdrs']['destination'] . "</td>" .
                            ...
                            "<td width=\"7%\" align=\"right\" style=\"padding-right:10px;\" class=\"gridcellborder\">" . $call[0]['totatcalls'] . " </td>" .
                            "<td width=\"7%\" align=\"right\" style=\"padding-right:10px;\" class=\"gridcellborder\">" . $call[0]['duration'] . " </td>" .
                            "<td width=\"7%\" align=\"right\" style=\"padding-right:10px;\" class=\"gridcellborder\">" . $call['currencies']['currency_name'] . " </td>" .
                            "<td width=\"7%\" align=\"right\" style=\"padding-right:10px;\" class=\"gridcellborder\">" . $call['cdrs']['superresrate'] . " </td>" .
                            "<td width=\"7%\" align=\"right\" style=\"padding-right:10px;\" class=\"gridcellborder\">" . $call['cdrs']['adminbuyrate'] . " </td>" .
                            "</tr>" .
                            "</table></td>" .
	...
	break;
     ...
     function GetTNList() {
          ...
          if ($this->Auth->user('role_id')) {
               ...
               // if super resseller viewing calls
               if ($this->Auth->user('role_id') == 2) {
                    $query = "SELECT dids.did, numberranges.name, numberranges.created  FROM dids
                                                                                                                   INNER JOIN numberranges ON numberranges.id=dids.numberrange_id
               ...
               if (count($calls) > 0) {
                    ...
                    foreach ($calls as $call) {
                         ...
                         if(strtotime($call['numberranges']['created']) > strtotime('1 month ago')){
                              $newgif = "<img src='/img/new.gif' >";
                         }
                         else{
                              $newgif = "";
                         }
                         ...
                         if ($counter >= 0 and $counter <= $half) {
                              $strCalls = $strCalls . "<tr" . $class . "><td class=\"grid1light\"><table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\" valign=\"top\">" .
                                      "<tr>";
                              $strCalls .= "<td width=\"50%\" height=\"29px\" align=\"left\" class=\"gridcellborder\" style=\"padding-left:10px;\">". $newgif . $call['numberranges']['name'] . "</td>" .
          ...
          if ($this->Auth->user('role_id')) {
               ...
               if ($this->Auth->user('role_id') == 2) {
                    $query = " 	SELECT DISTINCT (cdrs.start_stamp) ,cdrs.caller_id_number, cdrs.destination_number, cdrs.billsec, numberrange_name
								...
								WHERE dids.IsTestNumber=1 and (cdrs.start_stamp >= DATE_SUB(NOW(),INTERVAL 5 DAY)) 
								and cdrs.billsec > 0 
...
$conditonOnTerm="cdrs.isdaily=$payment_term and";	
          ...
          // admin viewing CDR
				...
				$query = "select CONCAT(users.`first_name`,' ',users.`last_name`) AS 'Name' , users.`login`,cdrs.caller_id_name, cdrs.caller_id_number, cdrs.destination_number, cdrs.start_stamp, (cdrs.billsec-cdrs.billsec * (cdrs.Addon/100))  as billsec, cdrs.answer_stamp, cdrs.numberrange_name ,cdrs.`operator_name` as 'Operator Name',CASE cdrs.`isdaily`  WHEN 1 THEN 'Daily' WHEN 0 THEN 'Weekly' WHEN 2 THEN 'Monthly' ELSE 'Any' END AS   'Payment Term', operator_ip, cdrs.`superresrate`, cdrs.adminbuyrate as 'Buy Rate', CASE cdrs.currency_id WHEN 1 THEN 'USD' WHEN 2 THEN 'EURO' WHEN 4 THEN 'GPB' ELSE 'Any' END AS 'Currency' from cdrs Inner Join users on users.id = cdrs.superresseler_id";
...
//          echo ($row); exit;
     ...
     function csvcalllogfromcds()
     {
          ...
          $query = "select caller_id_name,destination_number,operator_ip,start_stamp from unmatch_did_cdrs where unmatch_did_cdrs.billsec = 0 and answer_stamp='0000:00:00 00:00:00' and unmatch_did_cdrs.destination_number not in (select dids.did from dids) and unmatch_did_cdrs.start_stamp BETWEEN DATE_SUB(NOW(), INTERVAL 1 DAY) AND NOW();";
          $data = mysql_query($query) or mysql_error();

          $nn = date('y-m-d-h:m:s');
          $filename = "../../app/webroot/" . $nn . "calllogcdr.csv";
          ...
          $latest_filename = $nn . 'calllogcdr.csv';
//          echo $file;
          $file = "../../app/webroot/" . $nn . $latest_filename;
          if (file_exists($latest_filename)) {
...
$conditonOnTerm="cdrs.isdaily=$payment_term and";	
          ...
          $query = "select  CONCAT(users.`first_name`,' ',users.`last_name`) AS 'Name', users.`login`, cdrs.caller_id_name, cdrs.caller_id_number, cdrs.destination_number, cdrs.start_stamp, cdrs.billsec , cdrs.answer_stamp, cdrs.numberrange_name,cdrs.`operator_name` as 'Operator Name',CASE cdrs.`isdaily`  WHEN 1 THEN 'Daily' WHEN 0 THEN 'Weekly' WHEN 2 THEN 'Monthly' ELSE 'Any' END AS   'Payment Term', operator_ip, cdrs.`superresrate`, cdrs.adminbuyrate as 'Buy Rate',CASE cdrs.currency_id WHEN 1 THEN 'USD' WHEN 2 THEN 'EURO' WHEN 4 THEN 'GPB' ELSE 'Any' END AS 'Currency' from cdrs Inner Join users on users.id = cdrs.superresseler_id";
...
$conditonOnTerm="cdrs.isdaily=$payment_term and";	
          ...
          // admin viewing CDR
               ...
               $query = "SELECT users.login,users.first_name,cdrs.destination_number,Round(Round(SUM(cdrs.billsec)/60,2)-Round(SUM(cdrs.billsec)/60,2)*(cdrs.Addon/100),2) AS duration,
                    COUNT(cdrs.start_stamp) AS totatcalls, cdrs.numberrange_name as NumberRange, cdrs.operator_name FROM cdrs
                    INNER JOIN users ON cdrs.superresseler_id=users.id";
...
$conditonOnTerm="cdrs_archive.isdaily=$payment_term and";	
     ...
     function get_status() {
          ...
          function primary_ip_set(){
          $this->autoRender = false;
		$ip 		     = $_GET['ip'];
		$oper_id 	     = $_GET['oper_id'];
          $isprimary 	= $_GET['isprimary'];

          $isprimary1    = $isprimary === 'true' ? 1 : 0;
          if($isprimary === 'true'){
               $query1   = "SELECT * FROM operator_ips JOIN operators ON operators.`id` = operator_ips.`operator_id` where ip = '{$ip}' AND isprimary = 1";
               // $query2   = "SELECT * FROM operator_ips where operator_id = '{$oper_id}' AND isprimary = {$isprimary1}";
               $result   = mysql_query($query1);
               // $result2   = mysql_query($query2);

               // if(mysql_num_rows($result2) > 0){
               //      $r = mysql_fetch_assoc($result2);
               //      echo "This operator already have a primary IP.";
               //      return;
               // }
               ...
               if(mysql_num_rows($result)){
                    ...
                    echo "This IP is already to operator " . $r['name'];
                    return;
               }
               else{
                    $query = "UPDATE operator_ips SET isprimary = 1 WHERE ip = '{$ip}' AND operator_id = '{$oper_id}'";
                    if(mysql_query($query) or die(mysql_error())){
                         echo "Primary IP assigned.";
                         return;
                    }
                    else{
                         echo "Cannot Assign at this moment";
                         return;
                    }
               }
          }
          else{
               $query = "UPDATE operator_ips SET isprimary = 0 WHERE ip = '{$ip}' AND operator_id = '{$oper_id}'";
                    if(mysql_query($query) or die(mysql_error())){
