...
//          echo $file;
     ...
     function export_numberranges() {
          ...
          $query = "SELECT  operators.`name` as 'operater name', numberranges.`name` , route , ivrpath , currencies.`currency_name` , clilimit , buyingrate ,dailyrate, sellingrate AS 'Weekly Rate', `monthlyrate` AS 'Monthly Rate', maxduration , calllimit , maxdailyminutes,DATE(numberranges.`created`) AS 'Creation Date' FROM numberranges
     ...
     function getTNActiveCalls() {
          ...
          if ($this->Auth->user('role_id')) {
               ...
               if ($this->Auth->user('role_id') == 2) {
                    ...
                    INNER JOIN livecalls.dids_users ON (dids.id = dids_users.did_id) INNER JOIN livecalls.channels ON (channels.dest = dids.did) INNER JOIN numberranges ON (numberranges.id=dids.`numberrange_id`) where dids.IsTestNumber=1 and channels.direction='inbound';";
               }
               // if resseller viewing calls
               else if ($this->Auth->user('role_id') == 3) {
                    $query = "SELECT numberranges.`name`, channels.created,channels.dest,channels.cid_num,TIMEDIFF(time(now()),time(channels.created))as Duration,dids_users.superresseler_id,dids_users.resseller_id, dids_users.subresseller_id FROM livecalls.dids
...
$conditonOnTerm="cdrs.isdaily=$payment_term and";	
          ...
          // admin viewing CDR
               ...
               if ($subcustomer == -1 && $destination == -1) {
                    ...
                    $query = $query . " WHERE $conditonOnTerm cdrs.start_stamp BETWEEN " . "'" . $start . "'" . " and " . "'" . $end . ":59'" . " GROUP BY cdrs.destination_number,cdrs.superresseler_id,cdrs.isdaily,cdrs.`superresrate`,cdrs.`Addon`,cdrs.`adminbuyrate`, cdrs.`currency_id`";
               } else if ($subcustomer == -1 && $destination != -1) {
                    ...
                    $query = $query . " WHERE $conditonOnTerm  cdrs.superresseler_id=" . $subcustomer . " and cdrs.start_stamp BETWEEN " . "'" . $start . "'" . " and " . "'" . $end . ":59'" . " GROUP BY cdrs.destination_number, cdrs.superresseler_id,cdrs.isdaily,cdrs.`superresrate`,cdrs.`Addon`,cdrs.`adminbuyrate`,cdrs.`currency_id";
               } else if ($subcustomer != -1 && $destination != -1) {
                    $query = $query . " WHERE $conditonOnTerm  cdrs.superresseler_id=" . $subcustomer . " and cdrs.numberrange_name=" . "'" . $destinationtxt . "'" . " and cdrs.start_stamp BETWEEN " . "'" . $start . "'" . " and " . "'" . $end . ":59'" . " GROUP BY cdrs.destination_number, cdrs.superresseler_id,cdrs.isdaily,cdrs.`superresrate`,cdrs.`Addon`,cdrs.`adminbuyrate`,cdrs.`currency_id";
               }
          }
	...
	break;
     ...
     function GetTNList() {
		$sql1 = "";
		 if(isset($_GET["alpha"])) {
			$alpha = $_GET["alpha"];
			$sql1 = " and numberranges.name like '$alpha%' ";
		 }
		 else {
			$sql1 = " and numberranges.name like 'A%' ";
		 }
          $this->autoRender = false;
          if ($this->Auth->user('role_id')) {
               ...
               if ($this->Auth->user('role_id') == 2) {
                    $query = "SELECT dids.did, numberranges.name, numberranges.created  FROM dids
                              INNER JOIN numberranges ON numberranges.id=dids.numberrange_id
                              WHERE dids.IsTestNumber=1 $sql1 order by name asc ";
               }
               // if resseller viewing calls
               else if ($this->Auth->user('role_id') == 3) {
                    $query = "SELECT dids.did, numberranges.name, numberranges.created  FROM dids
