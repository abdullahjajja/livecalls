...
class UsersController extends AppController {
          ...
          if (!empty($this->data)) {
               if ($this->Auth->user('role_id') == 1) {
                    ...
                    if (isset($this->data['User']['email'])) {
                         ...
                         $email = $this->data['User']['email'];
                         $count = $this->User->find('count', array('conditions' => array('User.email' => $email, 'User.role_id' => '2', 'User.id !=' => $id)));
                         //  $count = $this->User->find('count', array('conditions' => array('User.role_id' => '2', 'User.email' => email)));
                         if ($count > 0) {
                              $this->Session->setFlash('Unable To Edit User, Email Already Exist . Try Again with some other emiail');
     ...
     function delete($id = null) {
          ...
          $ressellers = $this->User->find('all', array('conditions' => $conditions));
         
          if(!empty($ressellers)){
               foreach ($ressellers as $key => $value) {
                    $resseller_id = $value['User']['id'];

                     $this->User->updateAll(
                       array('User.isdeleted' => 1), array('User.id' => $value['User']['id'])
                    );

                    $condition2 = array('created_by' => $resseller_id);   
                    $sub_ressler = $this->User->find('all', array('conditions' => $condition2));
                    if(!empty($ressellers)){
                         foreach ($sub_ressler as $k => $v) {
           
                              $sub_resseller_id = $v['User']['id'];
                              $this->User->updateAll(
                                   array('User.isdeleted' => 1), array('User.id' => $sub_resseller_id)
                              );
                         }
                    }
               }
   ...
   function deactivate() {
     	if ($this->Auth->user('role_id') == 1) {
          ...
          $query="SELECT * FROM users JOIN roles ON roles.`id`=users.`role_id` WHERE users.`isdeleted`=1  and users.`role_id` = 2 ORDER BY login ASC";
         
          $results = $this->User->query($query, $cachequeries = false);
