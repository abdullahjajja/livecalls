          ...
          <script type="text/javascript">
               ...
               function getadminvoicecall(which) {
                    ...
                    if (which == 1) {
                         ...
                         var int_e = new Date(dateObj.getFullYear(), dateObj.getMonth(), 1);

                         //   var d = new Date(int_d - 1);

                         $("#txtstartdate").datepicker('setDate', int_e);
                         $("#txtenddate").datepicker('setDate', new Date());
                    }
                    if (which == 2) {

                         var int_e = new Date(dateObj.getFullYear(), dateObj.getMonth(), 0);
                         var int_s = new Date(dateObj.getFullYear(), dateObj.getMonth() - 1, 1);

                         $("#txtenddate").datepicker('setDate', int_e);
                         $("#txtstartdate").datepicker('setDate', int_s);
                    }
                    if (which == 3) {
                         var a = dateObj.daysMoreLess(-7);
                         $("#txtstartdate").datepicker('setDate', a);
                         $("#txtenddate").datepicker('setDate', new Date());
                    }
                    if (which == 4) {
                         var a = dateObj.daysMoreLess(-14);
                         var b = dateObj.daysMoreLess(-7);
...
//                    var e = document.getElementById("currency");
                    ...
                    if (userid > 0) {
                         $.ajax({
                              url: "<?php echo $this->Html->url(array('controller' => 'api', 'action' => 'GetOperatorInvoice')); ?>",
                              type: "GET",
     ...
     <script>
          function popup(id) {
               // alert(id);
               var url = '/operator_invoices/view/' + id;
