...
$usd=0;
	...
	while ( $r = mysql_fetch_array($result)){
		...
		$query2="
SELECT  numberranges.`buyingrate` AS superresrate,ROUND(ROUND(SUM(cdrs.billsec)/60,2),2) AS duration, 
Round(ROUND(ROUND(SUM(cdrs.billsec)/60,2)* numberranges.`sellingrate`),2) AS total, COUNT(cdrs.start_stamp) AS totatcalls ,
cdrs.`numberrange_name` ,currencies.id AS currenciesid , numberranges.`operator_id` FROM cdrs
...
JOIN numberranges ON numberranges.`name`=cdrs.`numberrange_name`
JOIN operators ON operators.`id`=numberranges.`operator_id`
WHERE numberranges.`operator_id` IN (SELECT operators.`id` FROM operators) AND cdrs.start_stamp BETWEEN CURDATE()+INTERVAL -1 MONTH AND SUBTIME( CONCAT(CURDATE(), ' 00:00:00'),'00:00:01')
...
ORDER BY operators.`name` ASC ,numberranges.`name` ASC ,duration DESC";
          ...
          $filename = "/var/www/html/setmyleads/app/webroot/operators_monthly_report.csv";
