...
$query5="SELECT users.id FROM users WHERE role_id=2";
//echo $query5;
...
WHERE (cdrs.superresseler_id =".$r[0]." ) AND cdrs.start_stamp BETWEEN '2017-07-01' +INTERVAL -1 MONTH AND SUBTIME( CONCAT('2017-07-01', ' 00:00:00'),'00:00:01') AND cdrs.`isdaily`=2 GROUP BY cdrs.`numberrange_name`,cdrs.`superresrate`,cdrs.`Addon`
		";
		...
		while ( $r2 = mysql_fetch_array($result2)){
			...
			//echo "<br>iN<br>";
				$c++;
				$q10="INSERT INTO `invoices`( `user_id`,`isdaily`) VALUES ({$r2['superresseler_id']},2) ";
			echo  $q10.'<br>';
...
$query="select users.`login` as 'User Login', round (ir.total_usd ,2)AS 'Total USD',  ROUND (ir.total_euro ,2)  AS 'Total EURO',  ROUND (ir.total_gbp,2) AS 'Total GBP'  ,concat(users.`first_name`,' ',users.`last_name`) as 'User Name' ,users.`preference`  from invoice_reports as ir
left JOIN invoices ON invoices.`id`=ir.invoice_id
left JOIN users ON users.id=invoices.`user_id`
where invoices.`date`>= curdate() and invoices.`isdaily`=2
...
$nn=date('ymd');
$filename = "/var/www/html/setmyleads/app/webroot/monthly.csv";
 //$filename = "../setmyleads/app/webroot/monthly.csv";
   $csv_terminated = "\n";
          $csv_separator = ",";
          $csv_enclosed = '"';
          $csv_escaped = "\\";
    ...
    $mail->Subject = 'Livecalls Daily Invoice Report';
    $mail->Body = 'Download the attached csv file';
	//$mail->AddAttachment('../setmyleads/app/webroot/monthly.csv', $fname);
	$mail->AddAttachment('/var/www/html/setmyleads/app/webroot/monthly.csv', $fname);
