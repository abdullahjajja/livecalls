...
/*$query1  = "SELECT REPLACE (cdrs.`caller_id_name`,'+','') AS 'caller_id_name',cdrs.`destination_number` , 
			...
			ORDER BY hits DESC";*/
$query1  = "SELECT REPLACE (unmatch_did_cdrs.`caller_id_name`,'+','') AS 'caller_id_name',unmatch_did_cdrs.`destination_number` ,
                        COUNT(*) AS 'hits',unmatch_did_cdrs.`operator_ip` AS 'oper_ips'
                        FROM unmatch_did_cdrs
                        WHERE billsec = 0 AND start_stamp  >= DATE_SUB(NOW(),INTERVAL 15 MINUTE)
                        GROUP BY REPLACE (unmatch_did_cdrs.`caller_id_name`,'+',''), destination_number
                        ORDER BY hits DESC";
...
$writer->writeToFile('/var/www/html/setmyleads/app/webroot/cdrs_access_temp.xlsx');
rename('/var/www/html/setmyleads/app/webroot/cdrs_access_temp.xlsx', '/var/www/html/setmyleads/app/webroot/cdrs_access.xlsx');
// print_r($superarray);
