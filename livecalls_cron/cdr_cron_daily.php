...
WHERE (cdrs.superresseler_id =".$r[0]." ) AND cdrs.start_stamp BETWEEN CURDATE()+INTERVAL -1 DAY AND SUBTIME( CONCAT(CURDATE(), ' 00:00:00'),'00:00:01') AND cdrs.`isdaily`=1 GROUP BY cdrs.`numberrange_name`,cdrs.`superresrate`,cdrs.`Addon`
		";
		...
		while ( $r2 = mysql_fetch_array($result2)){
			...
			//echo "<br>iN<br>";
				$c++;
				$q10="INSERT INTO `invoices`( `user_id`,`isdaily`) VALUES ({$r2['superresseler_id']},1) ";
			//echo  $q10.'<br>';
...
where invoices.`date`>= curdate() and invoices.`isdaily`=1";
$data=mysql_query($query);
while($r=mysql_fetch_array($data)){
//echo $r[0].' '.$r[1].'<br>';
}
$nn=date('ymd');
 $filename = "/var/www/html/setmyleads/app/webroot/daily.csv";
 //$filename = "../setmyleads/app/webroot/daily.csv";
   $csv_terminated = "\n";
          $csv_separator = ",";
          $csv_enclosed = '"';
          $csv_escaped = "\\";
	...
	$mail->AddAttachment('/var/www/html/setmyleads/app/webroot/daily.csv', $fname);
